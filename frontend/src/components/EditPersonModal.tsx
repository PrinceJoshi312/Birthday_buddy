...
import { X, User, Calendar, MessageSquare, AlertCircle, Check, ArrowLeft, Loader2, Bell, Clock, Edit3 } from 'lucide-react';
...
interface EditPersonModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onSave: (id: number, data: Partial<PersonInput>) => Promise<void>;
}
...
export const EditPersonModal: React.FC<EditPersonModalProps> = ({
  ...
  // Preload person values whenever modal opens or person changes
  useEffect(() => {
    if (person && isOpen) {
      setName(person.name || '');
      ...
      if (parsed) {
        setKnowsYear(parsed.hasYear);
        setSelectedMonth(parsed.month);
        setSelectedDay(parsed.day);
        if (parsed.hasYear && parsed.birthYear) {
          const mStr = String(parsed.month).padStart(2, '0');
          const dStr = String(parsed.day).padStart(2, '0');
          setFullDate(`${parsed.birthYear}-${mStr}-${dStr}`);
        } else {
          setFullDate('');
        }
      } else {
        setKnowsYear(true);
        setFullDate(person.birthday || '');
      }

      setRelationship((person.relationship as RelationshipType) || 'Friend');
      setNotes(person.notes || '');

      const reminders = (person.reminder_days || 'on_day,1_day_before')
        .split(',')
        .map((s) => s.trim())
        .filter(Boolean);
      setSelectedReminders(reminders.length > 0 ? reminders : ['on_day', '1_day_before']);

      setReminderTime(person.reminder_time || '09:00');
      setTouched({});
      setErrors({});
      setApiError('');
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setApiError(err.message || 'Failed to update birthday. Please try again.');
    } finally {
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 transition-opacity">
      <div className="relative w-full max-w-xl bg-white rounded-3xl shadow-2xl border border-warm-200/90 overflow-hidden transform transition-all animate-in fade-in zoom-in-95 duration-200 my-8">
        ...
        <div className="px-6 sm:px-8 pt-6 pb-5 bg-gradient-to-r from-purple-100/80 via-pink-100/70 to-amber-100/80 border-b border-warm-200/80 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium">
              Update birthday, relationship, notes, or reminder preferences
            </p>
          </div>
        ...
        <form onSubmit={handleSubmit} noValidate className="p-6 sm:p-8 space-y-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <label htmlFor="edit-person-notes" className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5">
